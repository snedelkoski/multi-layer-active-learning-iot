...
def random_sampling_reg(nr_exp=0, m=1000, accuracy=None):
    ...
    for i in range(0, len(x_unlabeled), len(x_unlabeled) // m):
        ...
        if (np.sqrt(mean_squared_error(x_predict, y_test))<0.06):
            break
        error_history_rs.append(np.sqrt(mean_squared_error(x_predict, y_test)))
    print (i)
